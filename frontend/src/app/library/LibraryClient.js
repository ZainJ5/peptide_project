...
export default function LibraryClient({ initialPeptides = [], initialCategories = [] }) {
  ...
  // Use server-fetched initial data while React Query is loading to avoid a flash
  const allPeptides = peptidesQuery.data?.data ?? initialPeptides;
  const isLoading = peptidesQuery.isLoading && initialPeptides.length === 0;

  const peptides = useMemo(() => {
    const source = allPeptides;
    const normalizedSearch = normalizeLooseSearch(search.trim());

    return source.filter((item) => {
      // Client-side type filter covers the case where initialPeptides (all types) is used
  ...
  }, [allPeptides, search, category, type]);

  const concernOptions = useMemo(() => {
    const backendCategories = categoryQuery.data?.categories ?? initialCategories;
    return backendCategories.length > 0 ? backendCategories : FALLBACK_CATEGORIES;
  ...
  const displayTotal = peptidesQuery.data?.total ?? initialPeptides.length;
        ...
        <section className="flex-1 min-w-0">
          <div className="mb-6 flex items-center justify-between gap-3">
            <p className="text-xs sm:text-sm font-medium text-slate-500">
              Showing <span className="font-bold text-slate-900">{displayTotal}</span> advanced protocols
            </p>
