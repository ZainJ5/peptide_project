...
// next/image wants a same-origin path; the API returns absolute URLs, so strip
// the origin for the optimizer (falls back to the raw value if already relative).
...
export default function VideosClient({ videos = [] }) {
  ...
  const playingVideo = filtered.find((v) => v.id === playingVideoId);
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((video) => (
            <Card key={video.id} className="overflow-hidden flex flex-col group hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.08)] transition-shadow">
              ...
              >
                  <Image
                    src={toLocalPath(video.thumbnailUrl)}
                    alt={video.title}
                    fill
                    loading="lazy"
