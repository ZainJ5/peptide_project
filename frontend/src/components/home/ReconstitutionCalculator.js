...
export default function ReconstitutionCalculator() {
  ...
  const calculate = () => {
    const amountMg =
      peptideUnit === "mcg"
        ? parseFloat(peptideAmount) / 1000
        : parseFloat(peptideAmount);
    const water = parseFloat(bacWater);
    const doseMg =
      doseUnit === "mcg"
        ? parseFloat(desiredDose) / 1000
        : parseFloat(desiredDose);

    if (
      !amountMg || !water || !doseMg ||
      amountMg <= 0 || water <= 0 || doseMg <= 0
    ) {
      setResult(null);
      return;
    }
  ...
  return (
    <section className="mx-auto max-w-7xl px-0 sm:px-6 w-full">
      <div className="rounded-2xl sm:rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        {/* Single flex-col column on mobile; two-column grid on lg */}
        <div className="flex flex-col lg:grid lg:grid-cols-2">

          {/* ── Info Panel ── */}
          <div className="p-6 sm:p-10 lg:p-14 lg:flex lg:flex-col lg:justify-center">

            <div className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-bold uppercase tracking-wider text-emerald-700 w-fit mb-4 sm:mb-6">
              <svg
            ...
            <p className="mt-3 text-sm sm:text-base text-slate-600 leading-relaxed max-w-lg">
              Calculate precise injection volumes, syringe units, and total
              doses from your vial. Enter your peptide details and get instant
              results.
            </p>

            {/* Steps — only shown on sm and up */}
            <div className="hidden sm:flex flex-col mt-7 space-y-4">
              {[
            ...
            <p className="mt-5 sm:mt-8 text-xs text-slate-400 leading-relaxed">
          ...
          {/* ── Form Panel ── */}
          <div className="border-t lg:border-t-0 lg:border-l border-slate-100 bg-slate-50/50 p-6 sm:p-10 lg:p-14">
            <div className="space-y-4 sm:space-y-5">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    min="0"
                    step="any"
                    className="h-12 flex-1 min-w-0 rounded-xl border border-slate-200 bg-white px-4 text-sm font-medium text-slate-800 outline-none transition-all placeholder:text-slate-400 focus:border-emerald-300 focus:ring-2 focus:ring-emerald-500/10"
                  />
                  <select
                    value={peptideUnit}
              ...
              <div>
                ...
                <input
                  ...
                  min="0"
                  step="any"
                  className="h-12 w-full rounded-xl border border-slate-200 bg-white px-4 text-sm font-medium text-slate-800 outline-none transition-all placeholder:text-slate-400 focus:border-emerald-300 focus:ring-2 focus:ring-emerald-500/10"
                />
              </div>
              ...
              <button
                onClick={calculate}
                className="h-12 w-full cursor-pointer rounded-xl bg-emerald-600 text-sm font-bold text-white shadow-lg shadow-emerald-600/20 transition-all hover:bg-emerald-700 hover:-translate-y-px hover:shadow-xl active:scale-[0.98]"
              >
                Calculate
              </button>
                  ...
                  <div className="divide-y divide-slate-100">
                    <div className="flex items-center justify-between px-4 sm:px-5 py-3.5">
                      ...
                      <span className="text-sm font-bold text-slate-900">
                        {result.volumeMl}{" "}
                        <span className="font-semibold text-emerald-600">mL</span>
                      </span>
                    </div>
                    <div className="flex items-center justify-between px-4 sm:px-5 py-3.5">
                      ...
                      <span className="text-sm font-bold text-slate-900">
                        {result.insulinUnits}{" "}
                        <span className="font-semibold text-emerald-600">units</span>
                      </span>
                    </div>
                    <div className="flex items-center justify-between px-4 sm:px-5 py-3.5">
                      ...
                      <span className="text-sm font-bold text-slate-900">
                        {result.totalDoses}{" "}
                        <span className="font-semibold text-emerald-600">doses</span>
                      </span>
                    </div>
                  </div>
                </div>
